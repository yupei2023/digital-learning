# B1 规范邮件与附件 Email & Attachments

第一学期 W2 · 5 分 · 🚫 **独立**（本单元全程不使用 AI；任务是一面镜子——评投入与格式规范，不评对错与文笔）· ≤ 120 分钟（年计划 v2.1 §3b 数字基本功行：微课 25 · 跟做 25 + 任务 50 · 日志 20）
Semester 1, W2 · 5 pts · 🚫 **Independent** (no AI anywhere; a mirror — scored on engagement and correctness of form, not on being right or on prose) · ≤ 120 min (Year Plan v2.1 §3b Digital Basics row: micro-lessons 25 · follow-along 25 + task 50 · log 20)

状态 Status 2026-08-30：**r1 —— 无人值守流水线整单元建成，本地提交，未推送。** 原本"待 ShiFu 批准"的每一项都按推荐选项落实，逐条列在下方"替 ShiFu 决定的事项"，请 ShiFu 过目。标 `[placeholder]` 的是真正未知的事实（导师姓名与邮箱、开课信；**自测正式题目已于 r3 定稿**）；视频的大陆播放待 ShiFu 实测。
...
状态 Status 2026-08-31（r3 · 站点视觉评审跟进 site-visual-upgrade review）：把微课 2「导师的下载文件夹」图接进计分任务——任务第 1 步在“改好名”之后多一句自检（把这个名字摆进那个文件夹，它拿到的是 ✓ 还是问号），w2 自评第 6 条同口径；figure 加锚点 `#downloads-figure`。分钟不变（任务 50、总 120），量规未动。「写邮件界面」图与「三封信的梯子」图**有意保留为插图**：前者的六个部位已被硬性要求与模板卡逐字复述，且它是全站唯一一张不能缩放的图（缩到 0.58× 附件标签会相撞）；后者是本周结构导览，学习者无从拿它判断自己的作业。落 _jonassen 站点评审 J1。
...
分值分配 Points：① 3（3·2·1·0）· ② 2（2·1.5·1·0）= 5；③ 不计分（完成 ✓）。① 分高，因为"信本身"是本周的核心动作；② 覆盖附件与按钮；③ 按年计划"T0 不计分"处理，导师只在登记表打 ✓、内容不看不评。

---

## 替 ShiFu 决定的事项 Decisions made on ShiFu's behalf — please review

流水线规则：每个待批准项取推荐选项。每一条都可以推翻；推翻后改 `tools/b1/` 里的片段并重跑 `python3 tools/build_b1.py` 即可。

1. **三条目标与 3 + 2 分值**：见上。第 ② 行"发展中"档为 **1.5 分**（2·1.5·1·0）——5 分周三行分不匀，取了半分档；导师算术略繁，如不喜欢半分可改 2·1·1·0 或把 ①② 改为 2+3。
2. **第 ③ 行不计分**：年计划 §2 明写"基线自测 T0（不计分）"，故自测与基线邮件只作"完成 ✓"，不占 5 分，也不设"证据不足"档（不影响 ①② 得分）。量规里保留此行是为了让三条目标与量规行一一对应（模板契约）。
3. **J2 的落地方式**：基线**必做通道 = 发给自己的邮件**（主题一字不差："我的第 2 周基线 My Week-2 baseline"，中英双语一行，保证 M5 页所写的邮箱搜索"我的第 2 周基线"能命中）；"抄进网站第一条日志"作为 W3 的**双保险**（本周还没有网站，与 M5 兜底卡的说法一致）。并要求学生在给导师的邮件里带一句"已完成自测并把结果发给了自己"，导师据此在登记表"基线自存"列打 ✓——不追、不看内容、不扣分。
4. **附件内容 = 一页"自我介绍卡"**：化名与编号 · 我为什么学这门课（整理 W1 那段反思即可，形成 M0→B1 的衔接）· 每周两个固定学习时段（W1 第 4 步定过）· 这学期最想学会的一件事。建议 PDF，接受 docx。
5. **课程文件命名规则**："课程-周-内容-化名"（数字化学习-W2-自我介绍-晨星.pdf / DL-W2-self-intro-Chenxing.pdf，中英示例并列）。此规则本周首次出现，B4（账号与文件整理周）扩展到整个文件世界——已在微课 2 里预告。
6. **邮件主题公式**："事由 · 我是谁"→ 本周"B1 自我介绍 · 化名（学生编号）"。全学期沿用（微课 1 给了后续示例）。
7. **隐私口径**：全课程（含邮件与附件）使用化名 + 学生编号，不用真实全名——导师按编号对应名单。学生误用真名只提醒不扣分（评分表有此条）。
8. **忘附件的补救规则**：自己当天补发并注明 → 不扣分；导师提醒后才补 → ② 行 1.5。把"得体补救"本身当作教学内容（微课 2、FAQ、量规、评分表四处一致）。
9. **"回复"按钮纳入 ② 行**：学生收到导师回信后用"回复"接一句"收到谢谢"——既练按钮又完成一次完整往来；没回不扣分，另写新邮件回的按评分表处理（其余全对则 2→1.5）。
10. **回复预期的口径**：24–48 小时（节假日除外）用于问题邮件（教纲原文）；本任务的**评分反馈在一周内**（与 M1–M5 反馈节奏一致）。两处在页面上分开写明，不混用。
11. **视频与分钟**：微课页短视频（0:36–2:41）**计入**各微课分钟（B 周预算有"微课 25"列，与 M5"全部选看"的处理不同）；单元首页 8 分钟入门视频与 want-more 长视频选看不计。`self-check.html` 按"工具页"豁免配视频（沿用 M5 决定 11 的归类，请 ShiFu 一并确认）。共用 10 个 B 站条目，全部经 view API 核实（2026-08-30，code 0、公开、单 P）；其中 BV1nY8n6KEgY 与 BV1Xgj46MEGG 无简介，内容**待 ShiFu 抽查**（media/scripts.md 已标）。
12. **无 reading.html**：蓝图 §13 未给 B 周映射读本章节，B 周页面结构（§3a）也不含读本位——导航为"单元首页 · 第 2 周 · 量规 · 自测"四项。情境引入改用校本教材里"1987 年中国第一封电子邮件"的史实段落（纯文字改写，未用旧图与任何提及前任职学校的句子）。
13. **自测题目与 M5 完全同题**：~~3 条样题逐字复用 M5 复测页；正式题目仍 `[placeholder]`~~ → **r3 已换成正式的 12 道题，与 M5 复测页逐字同题，见下方第 26 条。**
14. **自愿提交口径与 M5 r2 对齐**：T0 结果表发导师为**自愿、不影响成绩**（自测页第 5 步 + 伦理说明；评分表"不提醒、不追"）；必做的只有"发给自己"。
15. **跟做 = 彩排两幕**：热身（存导师联系人 + 给自己发带附件的测试邮件 + 练一次"回复"，15 分钟）+ 分箱活动"规范还是还差一步"（8 卡即时反馈，页内 JS，10 分钟）。理由：正式邮件不可撤回，先在自己身上练全套动作。
16. **自测题（第 6 节）6 题**：一周单元取 6 题（M0 一周 8 题、M5 两周 8 题之间取小）；正确项位置 1·3·0·2·1·0，分散无规律。
...
25. **Q2 · 样题基线的口径**：~~自测页警示卡加"开课后以正式题为准"一句~~ → **r3 已不再需要：正式题目就在页面上，警示卡撤除，换成"同一把尺子，量两次"的提示卡（第 26 条）。**
...
26. **A4 · 学习状态自测的正式题目已定稿（裁定：由团队代拟）**：决定 13、25 作废——`self-check.html` 的 `[placeholder]` 警示卡与 3 条样题换成**正式的 12 道题**（第 2 周基线 T0）。1–5 自评（1 = 完全不会 → 5 = 很有把握，还能讲给别人听），中英双语，覆盖第一学期真正教的十二件事，各对一处单元目标——① 邮件与附件（B1）· ② 截图录屏（B2）· ③ 建站与维护（B2/B6，含"发布后备份"）· ④ 翻译与核对（B3）· ⑤ 账号密码与隐私判断（B4）· ⑥ 检索、保存与记来源（B5）· ⑦ 数字工作台（B6）· ⑧ 为任务选工具（M4）· ⑨ 学习策略：自测与间隔复习（M1）· ⑩ 反思自己的学习（M1/M5）· ⑪ 判断一份网上资源适不适合我（M2/外部课程）· ⑫ 独自守住每周节奏（M3/外部课程线）。措辞参照 DigComp 式自评量表的**做法**（能力自陈、1–5、"我能……"），题目全部自拟，未逐字引用任何现成量表。与 M5 复测页**逐字同题**（构建后已用脚本比对：12 条 `<li>` 完全一致）。警示卡换成普通提示卡"同一把尺子，量两次"；结果仍只在本地显示、不保存不上传，基线邮件模板与主题字串（"我的第 2 周基线 My Week-2 baseline"）未动。**分钟不变**：w2 时间表"学习状态自测 12 分钟"照旧，全周仍 120（items=120 / printed=120）。全站仍只用"学习状态自测"这个名字。
27. **B6 · 结果表自愿提交（裁定：是）**：与 r2 现状一致，未改——必做的只有"发给自己"，把结果表发给导师是自愿的、不影响成绩。
28. **A3 · 登记表工具与栏目已定稿（裁定：由团队代选）**：腾讯问卷（备选金山表单）· 四栏 = 学生编号 · 单元与任务编号 · 作品链接 · 提交日期（后台自动）；规格写在 `mentor/welcome-letter-spec.md`「登记表规格」一节。**B1 不在表单的任务清单里**——第 2 周的作品就是发给导师的邮件本身，学生不登记，导师按评分表打 ✓；本单元页面因此无需改动。
...
### A · 学习者页面（`course-site/s1/b1-email-attachments/`，由 `tools/build_b1.py` 从 `tools/b1/*.html` 生成）
| # | 文件 | 材料 | 视频（微课页计入分钟，其余选看；均经 API 核实）|
|---|---|---|---|
...
| 8 | `task-1-self-intro-email.html` | 硬性要求 · 附件写什么 · 双语邮件模板 · 六步 · 评分摘要 · FAQ | BV1nY8n6KEgY（选看，微课 1 同段）|
| 9 | `self-check.html` | 学习状态自测（第 2 周基线）：正式 12 题（与 M5 复测页逐字同题，1–5 本地计分）+ "同一把尺子，量两次"提示卡 + **基线邮件模板（主题一字不差）** + 用法五步 + 伦理说明 | —（工具页，豁免）|
| 10 | `rubric.html` | ① ② 计分行四级 + ③ 完成行 · 规范要求 · 合成 | — |
| 11 | `quiz.html` + `quiz-data.js` | 6 题情境题；正确项 1·3·0·2·1·0 | — |
| 12 | `want-more.html` | 读 1 · 看 3 · 做 4；全部选做；无 AI 主题 | — |

### B · 媒体（`media/`）
| # | 文件 | 材料 |
|---|---|---|
| 13 | `media/scripts.md` | 素材来源表（10 个 B 站条目 + 教材史实段 + 自测 12 题 + 讲义；许可依据、访问状态）+ 四段微课录制脚本 + 不复用旧材料清单（FLA 邮箱演示视频、旧扫描图、直播课堂礼仪节）|
| 14 | `media/handouts/email-template.html` · `media/captions/README.md` · `media/alt-text.md` | 双语可打印讲义"规范邮件六件套"；字幕说明；替代文字清单（本单元无图片文件）|

### C · 导师材料（不入公开仓库：`../mentor/s1-b1-email-attachments/`，与 M0–M5 同一做法）
| # | 文件 | 材料 |
|---|---|---|
| 15 | `scoring-sheet.md` | 本周特别之处（第一封往来、无登记表）；登记表列（含"基线自存"✓ 列）；分值；20 余条常见情形（含忘附件补救、真名提醒、"文笔太好才需要核对"）；回信四要素；每周流程；边界；时间预算 |
| 16 | `feedback-bank.md` | 欢迎式开头 + 回应问题句 + ①② 行各级句式 + 基线提醒句 + 隐私与疑似 AI 询问句 + 期待下周的结尾 |

### 共享组件（仓库级）
- `assets/site.css` · `lang.js` · `checklist.js` · `quiz.js` · `progress.js`——B1 只复用，未改（分箱活动与学习状态自测的 JS 为页内脚本）。
- 生成器 `tools/build_b1.py`（项目级 `tools/`，不在公开仓库）：复制 build_m5.py 结构，NAV 为一周单元四项（单元首页 · 第 2 周 · 量规 · 自测）。改内容改 `tools/b1/` 片段，重跑 `python3 tools/build_b1.py`。
- 课程首页 `index.html` 在 M0 行后加了 B1 一行链接（本次提交唯一改动的模块外文件）。
...
| 检查 | 结果 |
|---|---|
| `tools/parity_check.py`（12 页 + 1 讲义 + 课程首页）| PASS · 0 issues |
| `tools/site_checks.py`（链接锚点 · 禁用词 · 页脚 · 无头 Chrome 双语 console · 390px 双语无溢出）| PASS · 14/14 页 |
| 必做分钟 = 120（w2 时间表逐项手工复核：脚本的 time-table 正则不匹配 `cards time-table` 双类名，故另行验算）| PASS · items 合计 120 · printed 120 |
| 禁用词扫描（S1 清单，含 mentor/s1-b1、tools/b1 与生成页）| PASS · 0 命中 |
| 页脚措辞 · "free" 不在页脚 | PASS |
| checklist.js / quiz.js / progress.js 行为（无头 Chrome DOM 核验）| PASS · w2 自测题渲染 6 题、清单状态行生成；index 进度条渲染；分箱 8 卡反馈位齐 |
| B 站 BV 号 API 核实 | 10 个条目均 code 0、公开、单 P（2026-08-30）；**大陆播放待 ShiFu 实测** |
| 非 B 站外链 | 本单元无站外链接（站内跨单元链接 M0 第 3/4 步均解析）|
| 自测题正确项位置 | 1·3·0·2·1·0 |

## 待 ShiFu 实测 / 过目 For ShiFu
- 上方"替 ShiFu 决定的事项"18 条，尤其第 1（分值与 1.5 半分档）、第 3（J2 基线通道与主题字串）、第 4（附件内容）、第 11（视频计入分钟 + 两段无简介视频抽查）、第 12（无读本页）。第 13、25 条已因 ShiFu 的 A4 裁定作废并落实（第 26 条）。
- 10 个 B 站视频的大陆播放实测。
- 占位（r3 后）：导师姓名与邮箱（开课信）· 开课信本身。**自测正式题目已不再是占位**（第 26 条）。
- 一致性提示：M5 的 w16 FAQ 与自测页兜底卡说"第 2 周页面会请你把结果发给自己**或**抄进网站第一条日志"——B1 实建为"必做发给自己 + W3 抄日志作双保险"，语义相容，无需改 M5；若 ShiFu 希望字面完全一致，可在 M5 兜底卡微调一词。

## 来源材料 Source material（2026-08-30 核对）
| 来源 | 位置 | 用法 |
|---|---|---|
| 年计划 v2.1 §2 W2 行 · §3a 数字基本功单元设计 · §3b 预算行 · §9 前后测 | 项目文件夹 | 目标、页面结构、120 分钟分配、自测名称与伦理 |
| 教纲 v7 W2 行 · 学习礼仪第 1 条 · AI 使用规范（🚫 独立定义）· 帮助与反馈条款 | 项目文件夹 | 核心问题、24–48 小时、🚫 口径、反馈节奏 |
| M5 评估 J2（M5 README 决定 19）+ M5 `self-check.html` 正式 12 题与兜底卡 | `course-site/s1/m5-semester-portfolio/` | 基线收尾设计；12 道题逐字同题；主题字串对齐 |
| 校本教材二稿"什么是在线学习"节（1987 第一封邮件史实段） | `../校本教材-在线学习/230217 (2稿) ….docx` | index 情境引入（纯文字改写；未用旧图与涉前任职学校句子） |
| M0 全单元（w1 预告、第 4 步邮箱、开课信占位措辞） | `course-site/s1/m0-orientation/` | 衔接：邮箱准备、W1 反思变附件素材、占位与导航口径 |
| M4 / M5 页面、README、build 脚本、评分表、句式库 | `tools/m4` `tools/m5`、`mentor/` | 结构与措辞模板；讲义做法；导师边界写法 |
| 教师旧视频《视频说明-FLA邮箱发邮件.mov》 | `../教学 Instuction/` | **未复用**（含前任职学校名与校内流程）；操作演示改用公开 B 站教程 |
| 未找到 | B 站无"中学生给课程导师发第一封带附件邮件"专讲 | 微课 1 靠文字版 + 对照示例 + 彩排活动；脚本备自录 |
