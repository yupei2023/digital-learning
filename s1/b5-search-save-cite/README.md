...
状态 Status 2026-08-31（r3 · 站点视觉评审跟进）：三件事。**(1) J3 —— 全站最弱的那张图**：删掉台阶图上方那段与它同序同词的正文（图示目录第一条底线：“若文字已经按同样顺序说过，删图或删字”），图本身改造成本周路线图——每一级新增一行「造它的微课 → 造好以后你手上真正多出来的东西」（微课 1·2 → 两三种更好的问法；微课 3 → 一个新的收藏夹文件夹；微课 4 → 三张来源卡＝本周任务），被删段落的依赖关系断言移进 figcaption；SVG 加 `dl-fit`，390px 上不再横切；figure 加锚点 `#ladder-figure`，w6 开场句指向它。**(2) 微课 4「一眼三看」两栏对照图接进计分任务**：任务第 3 步与硬性要求第 4 条改为——先把来源放进左栏还是右栏，让你这样放的那一看就是卡片上那句“因为”；同时给这张图加 `dl-fit`，因为它在 390px 上原本只显示左栏（右栏连表头都看不见），而任务现在要求两栏都用得上；一条中文小字由 15 字缩到 14 字以留出栏内边距。**(3)** 分钟不变（任务 65、总 120），量规未动。搜索回路图**保留为插图**：任务第 2 步已白纸黑字写着“至少换两种问法”，正是这张图的全部增量，再连一次只是装饰。落 _jonassen 站点评审 J1 与 J3。
