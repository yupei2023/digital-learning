...
| lesson-2 | 回译核对回路图（自绘内联 SVG：原句 → 译文 → 回译句 → 对照 → 回到起点；r3 按站点图示系统的标准"回路"图形重绘，中央标"一个回路 / 响了就再走"，底部一条带写明"对不上 = 值得怀疑处"与报告三行） | 页内代码 | 本课程自制 | 站内 |
| lesson-4 | 两栏对照图（自绘内联 SVG，r3 新增：左栏"它靠得住的地方"——说明性段落 / 常见词 / 短句 / 一个大意；右栏"它常失手的地方"——成语 / 专有名词 / 长句 / 要署名的文字） | 页内代码 | 本课程自制 | 站内 |
