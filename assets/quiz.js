/* Ungraded self-quiz with instant feedback.
   ...
   "Retake" re-renders the quiz in place (no page reload), so the learner stays at the same spot on the page. */
(function(){
    ...
    items.forEach(function(it,n){
      ...
      it.options.forEach(function(o,i){
        ...
        inp.addEventListener('change',function(){
          ...
          if(answered===items.length){var s=document.createElement('p');s.className='summary';s.appendChild(bi(['本次 '+correct+' / '+items.length+'。不计分，可重做。',correct+' of '+items.length+' this time. Ungraded — retake as often as you like.']));host.insertBefore(s,btn)}
        });
        q.appendChild(l);
      });
      q.appendChild(fb);host.appendChild(q);
    });
    var btn=document.createElement('button');btn.type='button';btn.className='ghost';btn.appendChild(bi(['重做','Retake']));
    btn.addEventListener('click',function(){render();host.scrollIntoView({block:'start'})});
